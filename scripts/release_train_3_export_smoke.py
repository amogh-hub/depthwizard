...
def main() -> None:
    """Accept RT3 export packaging against the already-built local project without inference."""
    manifest_path = PROJECT_DIR / "project-manifest.json"
    if not manifest_path.is_file():
        raise RuntimeError(
            "RT3 export smoke requires the local RT2/RT3 acceptance project at "
            f"{PROJECT_DIR}. Do not fabricate substitute evidence."
        )
    manifest = ProjectManifest.load(PROJECT_DIR)
    if manifest.artifact_path("dsm") is None and manifest.artifact_path("rdsm") is None:
        raise RuntimeError("acceptance project has no persisted elevation surface")
    ...
    with zipfile.ZipFile(first.bundle_path) as archive:
        ...
        if not any(name in names for name in ("products/dsm.tif", "products/rdsm.tif")):
            raise RuntimeError("export ZIP is missing the primary elevation product")
    ...
    acceptance = {
        ...
        "purpose": (
            "Release Train 3 deterministic scientific export integration acceptance; transport "
            "derivative only and not new model/elevation evidence"
        ),
        ...
        "scientific_boundary": (
            "The export copies hash-verified persisted project artifacts. It does not rerun or "
            "alter reconstruction, calibration, validation, mesh geometry, or analytical values."
        ),
    }
